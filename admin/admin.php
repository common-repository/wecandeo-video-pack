...
class WecanDeo_Admin {
    ...
    public function init_admin_page() {
        ...
        add_action('pre_get_posts', array(&$this, 'autoset_featured_image'));
    }
    ...
    public function autoset_featured_image($post_id) {
        ...
        global $post;
        /*
        ?><script>alert("autoset_featured_image start!!! -> post_id : <?= $post->ID ?>,<?= $_GET['message'] ?>");</script><?php
        */

        //특성이미지 수동 지정 체크 로직 시작
        ...
        //특성이미지 수동 지정 체크 로직 종료
        /*
                ?><script>alert("<?php echo $already_has_thumb?>");</script><?php
        ...
        */
        ...
        //echo ">>>>>>>>> empty(postID) : " .empty($post->ID) . "<br/>";
        //echo ">>>>>>>>> empty(message) : " .empty($_REQUEST['message']) . "<br/>";
        //echo ">>>>>>>>> empty(already_has_thumb) : " .empty($already_has_thumb) . "<br/>";

		//if (!$already_has_thumb)
        if(!empty($post->ID) && !empty($_REQUEST['message']) && empty($already_has_thumb)) {//글쓰기 업데이트이고 수동지정된 특성이미지가 없을 경우 동영상 썸네일 특성이미지 지정 로직 처리
            ...
            preg_match("/([a-zA-Z0-9\-\_]+\.|)wecandeo\.com\/video\/v\/(\?key\=)([a-zA-Z0-9\-\_]{46,80})/", $text, $matches2);

            error_log("wecandeo play url => {$matches2[0]} / video key => {$matches2[3]}");

            //echo ">>>>>>>>> content : " . $post->post_content . "<br/>";
            //echo ">>>>>>>>> matches2 : " . $matches2[0] . "<==>" . $matches2[1] . "<==>" . $matches2[2] . "<==>" . $matches2[3] . "<br/>";
            //echo ">>>>>>>>> isset(matches2[0]) : " . (isset($matches2[0])) . "<br/>";

            //if (!$matches2[0])
            if (isset($matches2[0]) != '1') return;//웹에티터 내에 위캔디오 동영상 관련 url이 없을 경우 썸네일 지정 종료

            //$imageurl = "http://{WCANDEO_API_DOMAIN}/video/thumbnail?k={$matches2[3]}";
            $imageurl = "http://" . WCANDEO_API_DOMAIN . "/video/thumbnail?k={$matches2[3]}";

            //echo ">>>>>>>>> imageurl result : " . $imageurl."<br/>";
            //return;

            // Generate thumbnail
            $thumb_id = $this->wecandeo_generate_post_thumb($imageurl, $post->ID);

            // If we succeed in generating thumg, let's update post meta
            //if ($thumb_id) {
            if ($thumb_id != '' && $thumb_id != null) {
                //echo ">>>>>>>>> thumb_id insert : ".$thumb_id."<br/>";
                //add_post_meta($post_id, '_thumbnail_id', $thumb_id, true);
                //update_post_meta($post_id, '_thumbnail_id', $thumb_id, true);
                update_post_meta($post->ID, '_thumbnail_id', $thumb_id, true);
            }
        }
    ...
    public function media_button() {
        ...
        <?php if ($wecandeo_api_key != '') : ?>
                    ...
                    //화면 해상도에 따른 업로드 본문 동적높이 지정 및 스크롤 지정(By 유현돈-2015-12-21)
                    document.getElementById("wecondeo_m_media_frame_content").style.height =
                        (document.getElementById("wecandeo_modal_div").clientHeight - (document.getElementById("wecandeo_upload_content_header").clientHeight + document.getElementById("wecandeo_upload_content_footer").clientHeight))+"px";
                    document.getElementById("wecondeo_m_media_frame_content").style.overflowY = "auto";
    ...
    public function upload_modal() {
        ...
        <div id="wecandeo_modal_div" class="wecandeo-modal-outer" >
            <div class="wecandeo-modal" title="wecandeo video">
                ...
                <div id="wecandeo_upload_content_header" class="wecondeo_m_media_frame_router">
                ...
                <div id="wecandeo_upload_content_div" class="wecondeo_m_media_frame">
                    <!-- 중앙 컨텐츠 영역 시작 -->
                    <div id="wecondeo_m_media_frame_content" class="wecondeo_m_media_frame_content">
                ...
                <div id="wecandeo_upload_content_footer" class="ui-dialog-buttonpane ui-widget-content ui-helper-clearfix">
